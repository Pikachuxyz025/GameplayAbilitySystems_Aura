...
#pragma region Boiler Plate: Source/Target
	...
	TagsToCaptureDefs.Add(Tags.Attributes_Resistance_Archane, DamageStatics().ArchaneResistanceDef);
...
#pragma endregion
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : Tags.DamageTypesToResistances)
	{
		...
		checkf(TagsToCaptureDefs.Contains(ResistanceTag), TEXT("TagsToCaptureDefs doesn't Contain Tag: [%s] in ExecCal_Damage"), *ResistanceTag.ToString());
...
void UExecCalc_Damage::DetermineDebuff(const FGameplayEffectCustomExecutionParameters ExecutionParams, const FGameplayEffectSpec& Spec, FAggregatorEvaluateParameters EvaluationParameters, const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		const float Typedamage = Spec.GetSetByCallerMagnitude(Pair.Key, false, -1);
